...
// PostCard component (can stay here or be moved to its own file if preferred)
const PostCard = ({ post, index }: { post: BlogPost, index: number }) => (
  <RevealOnScroll delay={index * 100}>
    <Link href={`/blogs/${post.slug}`} className="block bg-card rounded-lg shadow-lg overflow-hidden group h-full flex flex-col">
      <img src={post.imageUrl} alt={`Image for blog post titled: ${post.title}`} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
